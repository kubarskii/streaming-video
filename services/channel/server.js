...
// Router
const ChannelServiceRouter = require('./router');
...
const SERVICE_NAME = process.env.SERVICE_NAME || 'channel';

// Ensure unique port if PORT is already 3000
const ACTUAL_PORT = PORT === 3000 ? 3004 : PORT;
...
async function initializeContainer() {
    ...
    // Infrastructure - Configure connection pool for channel service
    const prismaClient = DatabaseConfig.getPrismaClient({ serviceType: 'channel' });
    const channelRepository = new PrismaChannelRepository(prismaClient);

    // Use Cases
    const createChannelUseCase = new CreateChannelUseCase(channelRepository, null);
    ...
    // Router
    const router = new ChannelServiceRouter(channelController);
...
async function main() {
    ...
    server.listen(ACTUAL_PORT, '0.0.0.0', () => {
        console.log(`✅ Channel Service listening on port ${ACTUAL_PORT}`);
